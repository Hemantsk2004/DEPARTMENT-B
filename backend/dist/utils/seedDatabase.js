...
const seedDatabase = async () => {
    try {
        ...
        const lecturers = await User_1.User.insertMany([
            {
        ...
        const students = await User_1.User.insertMany([
            {
                name: "Alice Williams",
        ...
        const courses = await Course_1.Course.insertMany([
            {
        ...
        await Course_1.Course.updateMany({ _id: { $in: [courses[0]._id, courses[1]._id] } }, { $addToSet: { students: { $each: [students[0]._id, students[1]._id] } } });
        await Course_1.Course.updateMany({ _id: { $in: [courses[1]._id, courses[2]._id] } }, { $addToSet: { students: { $each: [students[2]._id, students[3]._id] } } });
        await Course_1.Course.updateMany({ _id: { $in: [courses[2]._id, courses[3]._id] } }, { $addToSet: { students: { $each: [students[4]._id, students[5]._id] } } });
        await Course_1.Course.updateMany({ _id: { $in: [courses[4]._id, courses[5]._id] } }, { $addToSet: { students: { $each: [students[6]._id] } } });
        console.log("Students assigned to courses successfully.");
        // Create Materials
        console.log("Creating materials...");
        await Material_1.Material.insertMany([
            {
